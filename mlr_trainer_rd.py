...
class mlr_Trainer(Trainer):
    ...
    def _train_epoch(self):
        ...
        for batch in tqdm(self.dataset, total=len(self.dataset)):
            ...
            all_scores1 = all_features
            all_features = []
            outputs1 = torch.cat(all_scores, dim=0)
            outputs2 = torch.cat(all_scores1, dim=0)

            #break
            
            loss1, loss_info_dict1 = self._compute_dual_mlr_loss(
                all_features, all_scores)
            loss2, loss_info_dict2 = self._compute_dual_mlr_loss(
                all_features, all_scores1)
            rd_loss = self.mse_criterion(outputs1, outputs2)
            loss = loss1+loss2+rd_loss
            avg_recorder.add(loss_info_dict1, batch_size)
            avg_recorder.add(loss_info_dict2, batch_size)
            loss.backward()
            self.optimizer.step()
            ...
            self.tensorboard_writer.add_scalar(
                    'rd_loss/train', rd_loss.item(), step)
            for loss_info1, loss_info2 in zip(loss_info_dict1.items(), loss_info_dict2.items()):
                self.tensorboard_writer.add_scalar(
                    '{}/train'.format(loss_info1[0]), 0.5*(loss_info1[1]+loss_info2[1]), step)
    ...
    def _get_all_features_and_scores(
        ...
        all_features = []
        all_scores = []
        cluster_size = batch['attention_mask'].size()[1]
        
        #batch = {k: v.permute(1,0,2,3).reshape(1,3,-1,512) for k,v in batch.items()} #batch in cluster operation, not effective
        for cluster_name in range(cluster_size):
            ...
            outputs1 = self._get_features_and_scores_in_specific_cluster(
                batch, cluster_name)
            all_features.append(outputs1)
    ...
    def _get_features_and_scores_in_specific_cluster(
       ...
       # _, score_list = self.model(input_ids_in_cur_cluster, attention_mask_in_cur_cluster)
        ...
        return scores_in_cur_cluster #,score_list
    ...
    def _compute_dual_mlr_loss(self,
        ...
        dual_mlr_loss = score_mlr_loss #+ bce_loss
        loss_info_dict = {
    ...
    def _compute_bce_loss(self, all_scores):
        ...
        negative_scores = all_scores[-1] #torch.cat(all_scores[1:])
